...
const AgentWorkflow: React.FC = () => {
  ...
  const [integrationConfig] = useState<IntegrationConfig>({
    notion: {
  ...
  const [notionStatus, setNotionStatus] = useState<"idle" | "sending" | "sent">(
    "idle"
  );
  const [slackStatus, setSlackStatus] = useState<"idle" | "sending" | "sent">(
    "idle"
  );
  ...
  const handleRunWorkflow = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/integrations/gmail/fetch`, {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        ...
        console.error(
          "❌ Failed to fetch emails - Status:",
          response.status,
          "Error:",
          errorText
        );
        alert(
          `⚠️ Could not fetch emails from Gmail. Check browser console for details.`
        );
        emails = [];
      }
    } catch (error) {
      ...
      prev.map((agent) =>
        agent.id === "agent-1"
          ? {
              ...
              stats: {
                ...agent.stats,
                processed: agent.stats.processed + emails.length,
              },
              lastRun: "Just now",
            }
          : agent
      ...
      prev.map((agent) =>
        agent.id === "agent-2"
          ? {
              ...
              stats: {
                ...agent.stats,
                processed: agent.stats.processed + 1,
                success: agent.stats.success + 1,
              },
              lastRun: "Just now",
            }
          : agent
    ...
    try {
      const webhookUrl =
        integrationConfig.slack.webhookUrl ||
        localStorage.getItem("slack_webhook_url");

      if (!webhookUrl) {
        console.error("❌ No Slack webhook URL configured");
        alert(
          "⚠️ Slack webhook URL not configured. Please go to Settings and add your webhook URL."
        );
        setSlackStatus("idle");
      } else {
        // Call backend API to send to Slack
        const response = await fetch(
          `${API_BASE_URL}/integrations/slack/send`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
        ...
        if (response.ok) {
          ...
          alert(
            `❌ Failed to send to Slack: ${error.detail || "Unknown error"}`
          );
          setSlackStatus("idle");
        }
      }
      ...
      prev.map((agent) =>
        agent.id === "agent-3"
          ? {
              ...
              stats: {
                ...agent.stats,
                processed: agent.stats.processed + 2,
                success: agent.stats.success + 2,
              },
              lastRun: "Just now",
            }
          : { ...agent, status: "idle" as const }
  ...
  const handleTestNotion = () => {
    alert(
      `✅ Testing Notion Connection\n\nWorkspace: ${
        integrationConfig.notion.workspace
      }\nDatabase: ${
        integrationConfig.notion.database
  ...
  const handleTestSlack = () => {
    alert(
      `✅ Testing Slack Connection\n\nWorkspace: ${
        integrationConfig.slack.workspace
      }\nChannel: ${
        integrationConfig.slack.channel
...
⚠️ No emails found matching your filters.`;
    ...
    const urgent = emails.filter(
      (e) => e.category === "urgent" || e.labels?.includes("IMPORTANT")
    );
...
� EMAILS BY PERSON:\n`;
    ...
    emails.slice(0, 10).forEach((email, idx) => {
      ...
      const isUrgent =
        email.category === "urgent" || email.labels?.includes("IMPORTANT");
   ...
   💬 ${
    ...
    if (emails.length > 10) {
      summary += `\n... and ${
        emails.length - 10
      } more emails from other senders\n`;
    }
    ...
    <div className="agent-workflow">
      ...
      <div className="workflow-header">
        <div className="header-content">
          ...
          <p className="workflow-subtitle">
            Filter emails → Categorize → Summarize → Send to Notion & Slack
          </p>
        </div>
        <button
            ...
            <div className="filter-options">
              <button
                className={`filter-btn ${
                  filters.dateRange === "today" ? "active" : ""
                }`}
              ...
              <button
                className={`filter-btn ${
                  filters.dateRange === "yesterday" ? "active" : ""
                }`}
              ...
              <button
                className={`filter-btn ${
                  filters.dateRange === "last7days" ? "active" : ""
                }`}
              ...
              <button
                className={`filter-btn ${
                  filters.dateRange === "last30days" ? "active" : ""
                }`}
            ...
            <div className="filter-options">
              <button
                className={`filter-btn category ${
                  filters.categories.includes("work") ? "active" : ""
                }`}
              ...
              <button
                className={`filter-btn category ${
                  filters.categories.includes("urgent") ? "active" : ""
                }`}
              ...
              <button
                className={`filter-btn category ${
                  filters.categories.includes("personal") ? "active" : ""
                }`}
              ...
              <button
                className={`filter-btn category ${
                  filters.categories.includes("notification") ? "active" : ""
                }`}
      ...
      <div className="integration-config-panel">
        <div className="config-header">
          <h3>
            🔗 Integration Settings {showSettings ? "(OPEN)" : "(CLOSED)"}
          </h3>
          <button
              ...
              <div className="config-details">
                ...
                <div className="config-info">
                  ...
                  <div className="info-row">
                    ...
                    <span
                      className={`connection-status ${
                        integrationConfig.slack.connected
                          ? "connected"
                          : "disconnected"
                      }`}
                    >
            ...
            <div className="output-actions">
              <div className={`integration-status notion ${notionStatus}`}>
                ...
                <span className="status-text">
                  Notion ({integrationConfig.notion.database}):{" "}
                  {notionStatus === "sent"
                    ? "Sent"
                    : notionStatus === "sending"
                    ? "Sending..."
              ...
              <div className={`integration-status slack ${slackStatus}`}>
                ...
                <span className="status-text">
                  Slack ({integrationConfig.slack.channel}):{" "}
                  {slackStatus === "sent"
                    ? "Sent"
                    : slackStatus === "sending"
                    ? "Sending..."
