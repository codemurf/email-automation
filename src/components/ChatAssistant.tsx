...
What would you like me to help you with today?`,
  ...
  const shareToSlack = async (content: string) => {
    try {
      ...
      const response = await fetch('http://localhost:9000/api/v1/integrations/slack/send', {
        ...
        body: JSON.stringify({
          webhook_url: savedWebhookUrl || 'https://hooks.slack.com/services/YOUR/SLACK/WEBHOOK',
          text: slackText,
      ...
      if (response.ok) {
        ...
        if (result.success) {
          if (result.mock) {
            alert('⚠️ Slack webhook not configured. Please configure your webhook URL in settings.');
          } else {
      ...
      navigator.clipboard.writeText(slackText).then(() => {
        alert('⚠️ Could not send to Slack. Message copied to clipboard instead.');
      }).catch(() => {
  ...
  const suggestedPrompts = [
    { icon: '📧', text: 'Summarize today\'s emails', colorClass: 'from-blue-500' },
    { icon: '🚨', text: 'Show urgent emails', colorClass: 'from-red-500' },
    { icon: '📊', text: 'Email statistics', colorClass: 'from-green-500' },
    { icon: '🔍', text: 'Find work emails', colorClass: 'from-purple-500' }
  ];
  ...
  const formatMessageContent = (content: string) => {
    ...
    return lines.map((line, idx) => {
      ...
      if (line.includes('**') || line.trim().startsWith('•') || line.trim().startsWith('-') || line.trim().startsWith('*')) {
        ...
        return (
          <div key={idx} className={line.trim().startsWith('•') || line.trim().startsWith('-') || line.trim().startsWith('*') ? 'ml-4 my-1' : 'my-1'}>
            {parts.map((part, i) => {
      ...
      <div className={`chat-header ${theme}`}>
        <div className="header-content">
          <div className="header-left">
            ...
            <div>
              <h2 className={`header-title ${theme}`}>
                AI Email Assistant
              </h2>
              <p className={`header-subtitle ${theme}`}>
                Powered by DeepSeek • <span className="online-indicator">●</span> Online
              </p>
            </div>
          </div>
        ...
        <div className={`settings-panel ${theme}`}>
          <div className="settings-content">
            ...
            <div className="setting-item">
              <label className={`setting-label ${theme}`}>Slack Webhook URL</label>
              <input
                type="text"
          ...
          {messages.map((message, index) => (
            ...
            >
              ...
              <div className="message-content">
                ...
                <div className={`message-bubble ${message.role} ${theme} relative`}>
                  ...
                  <div className={`message-text ${theme} ${message.role === 'assistant' ? 'pr-8' : ''}`}>
        ...
        <div className="input-container">
          <div className={`input-wrapper ${theme}`}>
            ...
            <button
              ...
              className={`send-btn ${input.trim() && !isLoading ? 'active' : 'disabled'} ${theme}`}
            >
